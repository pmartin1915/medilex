...
src/
├── components/
│   ├── SwipeableCard.tsx         # Handles vertical swipe gestures (TikTok-style)
│   ├── ActionButtons.tsx         # 5 circular action buttons (right side)
...
### Swipe Gesture System (TikTok-Style)

**SwipeableCard.tsx:**
- Vertical-only swipes for navigation
- No horizontal swipes (removed entirely)
- Smooth rolling animation (like continuous sheet of paper)
- Uses React Native `PanResponder` + `Animated`

**Current Threshold:**
- Vertical: 80px (intentional swipe)
...
interface Props {
  ...
  onSwipeDown?: () => void;   // Previous card (no eval)
}
```

### Action Button System (TikTok-Style)

**ActionButtons.tsx:**
- 5 circular buttons stacked vertically on right side
- Positioned absolutely at right: 16px
- Button size: 56x56px (good touch target)
- Spacing: 16px between buttons

**Buttons (top to bottom):**
1. **Know It** (ThumbsUp, green) - Marks known, advances card
2. **Don't Know** (X, red) - Marks unknown, advances card
3. **Favorite** (Heart, pink) - Toggles favorite, stays on card
4. **Bookmark** (Bookmark, amber) - Toggles bookmark, stays on card
5. **Share** (Share2, teal) - Shares term + definition via native Share API
...
1. **Missing Error Handling**
   - Speech synthesis failures (needs try/catch)
   - Storage persistence errors (AsyncStorage)
   - Share failures (partially handled)
...
4. **Gesture Enhancements**
   - Visual feedback during swipe (card preview of next/prev?)
   - Haptic feedback on button presses
   - Button press animations (scale effect)
   - Consider inertia-based scrolling
...
5. **UX Improvements**
   ...
   - Show hint overlay on first use

6. **Performance**
   - Large term lists may cause lag
   - Consider virtualization for ReviewScreen
   - Optimize re-renders in SwipeableCard
   - ActionButtons re-render optimization

7. **Accessibility**
   - Screen reader support (labels added to buttons)
   - Keyboard navigation (web) - needs implementation
   - High contrast mode
   - Font scaling support
   - Button touch target sizes (currently good at 56px)
...
8. **Testing**
   ...
   - No E2E tests
   - Manual testing only
   - Need to test Share API on all platforms

9. **Code Quality**
   - Some hardcoded values (button sizes, spacing)
   - Threshold values (80px) could be configurable
...
1. **Add Error Handling** - Graceful failures, user feedback for all operations
2. **Add Loading States** - Skeleton screens, spinners, smooth transitions
3. **Polish Gestures** - Haptic feedback, button animations, visual hints
4. **Input Validation** - Handle edge cases, corrupted data, empty states
5. **Quality Improvements** - Accessibility enhancements, performance optimization
6. **Testing Suite** - Unit + E2E tests for all features
...
- Logic: `src/components/SwipeableCard.tsx` (vertical-only, line 5 for threshold)
- Usage: `src/screens/LearnScreen.tsx`
- Threshold: Line 5 in `SwipeableCard.tsx` (80px)

**Need to modify action buttons?**
- Component: `src/components/ActionButtons.tsx`
- Usage: `src/screens/LearnScreen.tsx` (handlers for each button)
- Button order: Know It, Don't Know, Favorite, Bookmark, Share (top to bottom)
...
### 2025-11-09: TikTok-Style Redesign (MAJOR UPDATE)
- **Removed horizontal swipes** entirely from SwipeableCard
- **Added ActionButtons component** with 5 circular buttons:
  - Know It (green, ThumbsUp) - evaluates and advances
  - Don't Know (red, X) - evaluates and advances
  - Favorite (pink, Heart) - toggles, stays on card
  - Bookmark (amber, Bookmark) - toggles, stays on card
  - Share (teal, Share2) - native share functionality
- **Simplified swipe gestures** to vertical-only (UP=next, DOWN=previous)
- **Added Share functionality** using React Native Share API
- **Rolling animation** - smooth vertical sliding (250ms) with spring snap-back
- **Increased swipe threshold** to 80px for more intentional swipes
- **Reduced card width** by 120px to accommodate action buttons
- **Updated LearnScreen** layout with absolute-positioned buttons

### Known State:
- ✅ TikTok-style UI: Implemented
- ✅ Vertical swipe: Working
- ✅ Action buttons: Implemented
- ✅ Share feature: Implemented (with error handling)
- ⚠️ Share on web: May need fallback testing
- ⚠️ Error handling: Partial (needs more coverage)
- ⚠️ Loading states: Missing
- ⚠️ Haptic feedback: Not implemented yet
